...
// Robusto: pula e-mails invalidos sem travar e marca quem ja recebeu
// (coluna F) para nao enviar duplicado se rodar de novo.
function enviarFalta5Dias() {
  var ss  = SpreadsheetApp.getActiveSpreadsheet();
  var aba = ss.getSheetByName('Inscrições') || ss.getSheets()[0];
  var COL_ENVIADO = 6; // coluna F: data em que a pessoa recebeu este disparo
  var valores = aba.getDataRange().getValues();
  ...
  var testes  = ['gustavo teste', 'teste', 'solys projetos', 'teste cores e local'];
  var reEmail = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  var jaEnviei = {}, total = 0, pulados = 0, erros = 0;

  // cabecalho da coluna de controle (cria uma vez)
  if (String(valores[0][COL_ENVIADO - 1] || '').trim() === '') {
    aba.getRange(1, COL_ENVIADO).setValue('Enviado 5 dias').setFontWeight('bold');
  }
  ...
  for (var i = 1; i < valores.length; i++) {
    var nome  = String(valores[i][1] || '').trim();
    // limpa espacos e caracteres invisiveis do e-mail
    var email = String(valores[i][3] || '').replace(/[​-‍﻿]/g, '').replace(/\s+/g, '');
    var chave = email.toLowerCase();
    var jaMarcado = String(valores[i][COL_ENVIADO - 1] || '').trim();
    if (jaMarcado) continue;                                  // ja recebeu antes
    if (testes.indexOf(nome.toLowerCase()) !== -1) continue;  // pula testes
    if (!reEmail.test(email)) { pulados++; Logger.log('E-mail invalido (pulado): "' + email + '" (linha ' + (i + 1) + ')'); continue; }
    if (jaEnviei[chave]) continue;                            // repetido no mesmo envio
    jaEnviei[chave] = true;
    try {
      MailApp.sendEmail({ to: email, subject: assunto, htmlBody: html, name: 'Mulheres Curadas', inlineImages: { arte: imagem } });
      aba.getRange(i + 1, COL_ENVIADO).setValue(new Date()); // marca como enviado
      total++;
      Utilities.sleep(300);
    } catch (err) {
      erros++;
      Logger.log('Falha ao enviar para "' + email + '" (linha ' + (i + 1) + '): ' + err);
    }
  }
  Logger.log('Disparo "Faltam 5 dias": enviados=' + total + ' | invalidos pulados=' + pulados + ' | erros=' + erros);
}

// ===== TESTE: envia o "Faltam 5 dias" só para os e-mails abaixo =====
// Coloque aqui os e-mails para testar. Rode esta função para conferir
// como fica, ANTES de disparar para a lista toda (enviarFalta5Dias).
function testarFalta5Dias() {
  var EMAILS_TESTE = ['solysprojetos@gmail.com']; // <-- troque/adicione e-mails aqui
  var IMG_URL = 'https://mulherescuradas.institutoabner.com.br/falta-5-dias-mulheres-curadas.jpg';
  var imagem  = UrlFetchApp.fetch(IMG_URL).getBlob().setName('falta5dias.jpg');
  var assunto = '🌸 [TESTE] Faltam apenas 5 dias para o Mulheres Curadas!';
  var html =
    '<div style="font-family:Arial,sans-serif;max-width:600px;margin:auto;background:#170f0d;color:#f4eef6;border-radius:16px;overflow:hidden">' +
      '<img src="cid:arte" style="width:100%;display:block" alt="Faltam 5 dias - Mulheres Curadas">' +
      '<div style="padding:26px 24px">' +
      ...
      '</div>' +
    '</div>';
  for (var i = 0; i < EMAILS_TESTE.length; i++) {
    MailApp.sendEmail({ to: EMAILS_TESTE[i], subject: assunto, htmlBody: html, name: 'Mulheres Curadas', inlineImages: { arte: imagem } });
    Utilities.sleep(300);
  }
  Logger.log('Teste enviado para: ' + EMAILS_TESTE.join(', '));
}
